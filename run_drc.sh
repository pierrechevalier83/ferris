...
DRC_OUTPUT=$(docker run -t -e VARIANT=$VARIANT -e RUST_LOG -v $WORKDIR:/workdir pierrechevalier83/kicad_cli bash -c 'kicad_cli run-drc $VARIANT/ferris.kicad_pcb --headless --drc-timeout-in-s 60')
